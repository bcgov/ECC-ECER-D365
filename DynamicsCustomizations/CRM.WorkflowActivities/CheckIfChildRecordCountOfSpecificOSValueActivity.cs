...
{
    /// <summary>
    /// Search to check if records exists for a particular Option Set attribute value
    /// </summary>
    public class CheckIfChildRecordCountOfSpecificOSValueActivity : CodeActivity
    {
        [RequiredArgument]
        ...
        [RequiredArgument]
        [Input("Filter By Attribute Key")]
        public InArgument<string> AttributeKeyInput { get; set; }

        [RequiredArgument]
        [Input("Search By OptionSet Attribute Name")]
        public InArgument<string> OptionSetAttributeNameInput { get; set; }

        [RequiredArgument]
        [Input("Search By OptionSet Value")] 
        public InArgument<int> OptionSetSearchValueInput { get; set; }
        ...
        protected override void Execute(CodeActivityContext activityContext)
        {
            try
            {
                ...
                string optionSetAttributeName = OptionSetAttributeNameInput.Get(activityContext).ToString();
                int optionSetValue = OptionSetSearchValueInput.Get(activityContext);
                ...
                if (!string.IsNullOrEmpty(entityName) && !string.IsNullOrEmpty(fieldName))
                {
                    ...
                    query.Criteria.AddCondition(optionSetAttributeName, ConditionOperator.Equal, optionSetValue);
                    if (!includeInactive)
                    {
