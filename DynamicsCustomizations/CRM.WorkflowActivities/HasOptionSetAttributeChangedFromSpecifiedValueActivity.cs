...
{
    public class HasOptionSetAttributeChangedFromSpecifiedValueActivity : CodeActivity
    {
        [RequiredArgument]
        ...
        protected override void Execute(CodeActivityContext activityContext)
        {
            try
            {
                ...
                var oldValueEqualsSpecified = OldValue != null && OldValue.Value == specifiedValue;
                if (OldValue != null)
                {
                    Tracing.Trace($"Old Value {OldValue.Value} is specified value {specifiedValue}?  {oldValueEqualsSpecified}");
                }
                var isChangedFromSpecifiedValue = changed && oldValueEqualsSpecified;
                Tracing.Trace($"Value changed: {isChangedFromSpecifiedValue}");

                HasChangesOutput.Set(activityContext, changed && isChangedFromSpecifiedValue);
