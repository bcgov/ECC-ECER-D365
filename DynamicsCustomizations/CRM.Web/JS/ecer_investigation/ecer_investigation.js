...
ECER.Jscripts.Investigation = {
  onLoad: function (executionContext) {
    ...
    ECER.Jscripts.Investigation.showHideReasonForPriorityAssignment(
      executionContext
    );
    ...
    ECER.Jscripts.Investigation.onChangeInvestigationOutcomeInfo(
      executionContext
    );
...
// ECER-5499   (new function 2)   	
  ...
  hideBPFResponseDate: function (executionContext) {
    ...
    var reportInProgressControl = formContext.getAttribute(
      "ecer_reportinprogress"
    );
    ...
    crm_Utility.showHide(
      executionContext,
      !show,
      "header_process_ecer_requestedresponsebydate"
    );
  },
  ...
  onChangeTDADFacility: function (executionContext) {
    ...
    formContext.ui.tabs
      .get("tab_2")
      ?.sections?.get("tab_Active_section_9")
      ?.setVisible(
        tdadFacility != null && tdadFacility.getValue() === 621870000
      );
  },
  ...
  getCurrentCertificateInfo: function (executionContext) {
    ...
    let currentCertificate = formContext.getAttribute(
      "ecer_currentcertificate"
    );

    if (currentCertificate === null || currentCertificate.getValue() === null)
      return;

    let currentCertificateId = currentCertificate
      .getValue()[0]
      .id.replace("{", "")
      .replace("}", "");
    ...
    Xrm.WebApi.retrieveRecord(
      ...
      "?$select=ecer_expirydate,ecer_certificatelevel,ecer_certificatenumber,statuscode"
    ).then(
      function (result) {
        ...
        if (result.hasOwnProperty("ecer_certificatelevel"))
          formContext
            .getAttribute("ecer_certificationtype")
            ?.setValue(result["ecer_certificatelevel"]);

        if (result.hasOwnProperty("statuscode"))
          formContext
            .getAttribute("ecer_certificatestatus")
            ?.setValue(
        ...
        if (result.hasOwnProperty("ecer_expirydate")) {
          ...
          let expiryDateVal = new Date(
            result["ecer_expirydate@OData.Community.Display.V1.FormattedValue"]
          );
        ...
        if (result.hasOwnProperty("ecer_certificatenumber")) {
          formContext
            .getAttribute("ecer_certificatenumber")
            ?.setValue(result["ecer_certificatenumber"]);
        }
      },
      function (error) {
  ...
  getApplicantInfo: function (executionContext) {
    ...
    // From Certificate
    var currentCertificate = formContext.getAttribute(
      "ecer_currentcertificate"
    );
    ...
    // From Transcript
    var educationalInstitution = formContext.getAttribute(
      "ecer_educationalinstitution"
    );
    ...
    Xrm.WebApi.retrieveRecord(
      "contact",
      contactId,
      "?$select=birthdate,ecer_clientid,emailaddress1,ecer_isunder19,address1_composite,telephone1"
    ).then(
      function success(result) {
        ...
        var birthdayVal = new Date(
          result["birthdate@OData.Community.Display.V1.FormattedValue"]
        );
    ...
    // Get Certificate
    var certificateIdPromise = Xrm.WebApi.retrieveMultipleRecords(
      "ecer_certificate",
      "?$select=ecer_certificateid,ecer_certificatelevel,ecer_certificatenumber,ecer_expirydate,ecer_name,statuscode&$filter=_ecer_registrantid_value eq " +
        contactId +
      ...
      function success(results) {
        ...
        if (results.entities.length > 0) {
          let certificationHierarchyData =
            ECER.Jscripts.Investigation.Data.CertificationHierarchy();
          ...
          if (allActive.length > 0) {
            ...
            let highestActiveLevel3s = allActive.filter(function (f) {
              return f.ecer_certificatelevel.includes(
                certificationHierarchyData.Level3
              );
            });

            let highestActiveLevel2s = allActive.filter(function (f) {
              return (
                f.ecer_certificatelevel === certificationHierarchyData.Level2
              );
            });

            let highestActiveLevel1s = allActive.filter(function (f) {
              return certificationHierarchyData.Level1.includes(
                f.ecer_certificatelevel
              );
            });
          ...
          certificateStatus.setValue(
            result["statuscode@OData.Community.Display.V1.FormattedValue"]
          );

          var expiryDateVal = new Date(
            result["ecer_expirydate@OData.Community.Display.V1.FormattedValue"]
          );
          expiryDate.setValue(expiryDateVal);
          expiryDate.setSubmitMode("always");
          ...
          ECER.Jscripts.Investigation.setValueToLookup(
            currentCertificate,
            result["ecer_certificateid"],
            result["ecer_name"],
            "ecer_certificate"
          );
    ...
    Promise.all([certificateIdPromise]).then(function (values) {
      ...
      if (certificateId !== null) {
        Xrm.WebApi.retrieveMultipleRecords(
          "ecer_transcript",
          "?$select=ecer_educationinstitutionfullname&$filter=ecer_Applicationid/_ecer_certificateid_value eq " +
            certificateId
        ).then(
          function success(results) {
            console.log(results);
            if (results.entities.length > 0) {
              console.log(
                "Eductional institutes for that cert id: ",
                results.entities
              );
              var result = results.entities[0];
              educationalInstitution.setValue(
                result["ecer_educationinstitutionfullname"]
              );
            }
          },
          function (error) {
    ...
    Xrm.WebApi.retrieveMultipleRecords(
      "ecer_application",
      "?$select=ecer_applicationid,ecer_name,statecode&$filter=(_ecer_applicantid_value eq " +
        contactId +
        " and statecode eq 0)&$orderby=createdon desc"
    ).then(
      function success(results) {
        console.log(results);
        if (results.entities.length > 0) {
          ...
          console.log(
            "No current open Application for Registrant:" + contactId
          );
          return null;
        }
      },
  ...
  lockComplaintInfo: function (executionContext) {
    ...
    formContext.ui.tabs
      .get("{1328d0d8-b96a-4553-a84d-fb0fb98086db}")
  ...
  displayFindingsOrAllegations: function (executionContext) {
    ...
    var findingsSection = formContext.ui.tabs
      .get("{1328d0d8-b96a-4553-a84d-fb0fb98086db}")
  ...
  referToInvestigationButtonClick: function (PrimaryControl) {
    ...
    var recordId = PrimaryControl.data.entity
      .getId()
      .replace("{", "")
      .replace("}", "");
    ...
    Xrm.Navigation.navigateTo(pageInput, navigationOptions).then(
      function success(returnValue) {
        ...
        if (returnValue.returnValue == "EmptyDescription") {
          // Reopen input dialog
          ECER.Jscripts.Investigation.referToInvestigationButtonClick(
            PrimaryControl
          );
        ...
        Xrm.WebApi.retrieveMultipleRecords(
          "workflow",
          "?$select=_ownerid_value,primaryentity,name,uniquename&$filter=name eq 'ECER - Application Refer to investigation action'"
        ).then(
          function success(results) {
            console.log(results);
            ...
            ECER.Jscripts.Investigation.executeReferenceVerificationAction(
              "ecer_applications",
              "ecer_ECERRefertoinvestigationaction",
              parameters,
              recordId,
              ownerid
            );
  ...
  workExperienceReferenceVerification: function (PrimaryControl) {
    ...
    Xrm.Navigation.navigateTo(pageInput, navigationOptions).then(
      function success(returnValue) {
        ...
        if (returnValue.returnValue == "EmptyDescription") {
          // Reopen input dialog
          ECER.Jscripts.Investigation.workExperienceReferenceVerification(
            PrimaryControl
          );
        ...
        Xrm.WebApi.retrieveMultipleRecords(
          ...
          function success(results) {
            ...
            ECER.Jscripts.Investigation.executeReferenceVerificationAction(
              ...
              "ecer_ECERWorkExperienceReferenceVerification",
              parameters,
              recordId,
              ownerid
            );
  ...
  characterReferenceVerification: function (PrimaryControl) {
    ...
    Xrm.Navigation.navigateTo(pageInput, navigationOptions).then(
      function success(returnValue) {
        ...
        if (returnValue.returnValue == "EmptyDescription") {
          // Reopen input dialog
          ECER.Jscripts.Investigation.characterReferenceVerification(
            PrimaryControl
          );
        ...
        Xrm.WebApi.retrieveMultipleRecords(
          ...
          function success(results) {
            ...
            ECER.Jscripts.Investigation.executeReferenceVerificationAction(
              "ecer_characterreferences",
              "ecer_ECERCharacterReferenceVerification",
              parameters,
              recordId,
              ownerid
            );
  ...
  executeReferenceVerificationAction: function (
    entityType,
    actionName,
    parameters,
    recordId,
    ...
    fetch(
      ...
      .then(function success(response) {
        if (response.ok) {
          // Confirmation dialog
          var confirmStrings = {
            text: "Reference has been successfully referred to Investigations for verification",
            title: "Confirmation",
          };
  ...
  showHideParallelProcess: function (executionContext) {
    ...
    if (
      formContext.getAttribute("ecer_parallelprocess").getValue() == 621870000
    ) {
      if (tab) {
        tab.setVisible(true);
      }
  ...
  showHideImmediateActions: function (executionContext) {
    ...
    if (
      formContext
        .getAttribute("ecer_immediateinvestigatorassignment")
        .getValue() == 621870000
    ) {
      if (tab) {
        tab.setVisible(true);
  ...
  showHideReasonForPriorityAssignment: function (executionContext) {
    ...
    var val = formContext
      .getAttribute("ecer_recommendforpriorityassignment")
      .getValue();

    // val = Yes
    if (val == 621870000) {
      formContext
        .getControl("ecer_reasonforpriorityassignment")
        .setVisible(true);
    } else {
      formContext
        .getControl("ecer_reasonforpriorityassignment")
